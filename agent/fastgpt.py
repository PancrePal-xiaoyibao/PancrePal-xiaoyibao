...
if __name__ == "__main__":
    # 示例1: 不使用上下文功能（chat_id为None）
    messages_full_context = [
        {"role": "system", "content": "你是一个电影专家"},
        {"role": "user", "content": "请介绍一下《泰坦尼克号》"},
        {"role": "assistant", "content": "《泰坦尼克号》是1997年的经典电影..."},
        {"role": "user", "content": "导演是谁"}
    ]

    # 示例2: 使用上下文功能（只有最后一个message会被使用）
    ...
    def test_without_context():
        ...
        try:
            result = chat_completions(
                ...
                app_id="6868f94e4687731ebc930174",
                chat_id=None,  # 不使用上下文
                detail=True,
                variables=variables
            )
            print("不使用上下文功能结果:", json.dumps(result, ensure_ascii=False, indent=2))
        except Exception as e:
            print(f"不使用上下文功能错误: {e}")
            traceback.print_exc()
    ...
    def test_with_context():
        ...
        try:
            result = chat_completions(
                ...
                app_id="6868f94e4687731ebc930174",
                chat_id="unique_chat_id_123",  # 使用上下文
                response_chat_item_id="response_item_id_456",
                detail=True,
                variables=variables
            )
            print("使用上下文功能结果:", json.dumps(result, ensure_ascii=False, indent=2))
        except Exception as e:
            print(f"使用上下文功能错误: {e}")
            traceback.print_exc()
