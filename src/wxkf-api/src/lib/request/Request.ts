...
export default class Request {
  ...
  constructor(ctx, options: RequestOptions = {}) {
    ...
    if (
      this.type.startsWith("text/xml") ||
      this.type.startsWith("application/xml")
    )
      this.body = util.parseXML(ctx.request.body) || {};
    else
  ...
  validate(key: string, fn?: Function) {
    try {
      ...
      if (fn) {
        if (fn(value) === false) throw `[Mismatch] -> ${fn}`;
      } else if (_.isUndefined(value)) throw "[Undefined]";
    } catch (err) {
