...
const QINGYAN_GLMS_REFRESH_TOKEN =
  environment.envVars["QINGYAN_GLMS_REFRESH_TOKEN"];
// 模型名称
...
async function createCompletion(
  assistantId = DEFAULT_ASSISTANT_ID,
  messages: any[],
  refreshToken = QINGYAN_GLMS_REFRESH_TOKEN,
  refConvId = "",
  retryCount = 0
) {
  return (async () => {
    ...
    content = messages.reduce((content, message) => {
      if (_.isArray(message.content)) {
        return message.content.reduce((_content, v) => {
          ...
          return _content + (v["text"]["content"] || "") + "\n";
        }, content);
      }
